...
public interface IView {
  ...
  /**
   * Updates the list of available shapes. Not ideal, would like a single point of control where
   * changing the model would automatically update the list. At the least the controller is acting
   * as a single point of control? Not sure if this is the best way to do this.
   *
  ...
  /**
   * Takes in the transformations to display. It isn't ideal that we had to add these methods to our
   * interface. It doesn't make sense and it tightly couples our interfaces. We tried extending the
   * interface, using a decorator, but it came back to having to promise the functionality here. The
   * main issue was: we passed data to the views through an immutable model. When the controller
   * made changes to the model, although the view can't make any changes, it was able to observe
   * these changes. The provider's view does not contain a ro model, but the values themselves. When
   * we adapted their Transformation with our Motion, the reference was lost. Our interface needed
   * functionality for being able to update the view after every change the controller makes.
   *
