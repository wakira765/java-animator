...
/**
 * <p>Our model and the provider's model stored data differently. For the most part it's the
 * same,our motion contained the values from one key frame to the other, by storing these in two
 * State classes. Each State representing a keyframe. It seems the provider put all the values in
 ...
 * adapting was mostly simple method calls.</p>
 *
 * <p>Main issue we faced: we passed data to the views through an immutable model. When the
 * controller made changes to the model, although the view can't make any changes, it was able to
 * observe these changes. The provider's view does not contain a model, but the values themselves.
 ...
 * specifically in the docs for the IView interface</p> .
 */
